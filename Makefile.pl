...
$LIBS = "-shared -lSDL -lglut -lGL -lGLU -O3 -Wall -fPIC";
$LOCATION = "clibs";
...
sub compileFile  {
    ...
    if (!(-e "$LOCATION/$compiled") or (-M "$LOCATION/$loc" <= -M "$LOCATION/$compiled")) {
        print "gcc $libs $LOCATION/$loc -o $LOCATION/$compiled\n";
        `gcc $libs $LOCATION/$loc -o $LOCATION/$compiled`;
    }
...
compileFile("GLee.c", "GLee.o", "-c -fPIC -O3");
